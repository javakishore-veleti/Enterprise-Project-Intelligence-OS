"""Mitigation Planning reporter (LLM).

Consolidates the finding set into a prioritized mitigation plan (a RiskReport of
kind MITIGATION).
"""
...
from mitigation_planning.prompts import SYSTEM_PROMPT, build_user_prompt
...
class MitigationPlanningReporter(Reporter):
    agent_key = AGENT_KEY
...
def build(model: str = DEFAULT_MODEL, framework: str = "langgraph") -> MitigationPlanningReporter:
    return MitigationPlanningReporter(model=model)
