...
- **`Agents/` — the specialist-agent package (`epi-agents`).** `agent_core` = framework-agnostic port (`EvidencePackage → RiskFinding[]`) + shared deterministic scoring. **All 7 evidence→findings detector agents implemented** (each with a LangGraph adapter, stub adapters, `registry.py` toggle): `schedule_risk`, `quality_risk`, `project_status_tracking` (delivery), `dependency_risk`, `resource_risk`, `backlog_health`, `delivery_forecasting` (release-readiness). `schedule_risk` also has a real **`openai_agents` adapter** (OpenAI Agents SDK → LiteLLM → Claude) — proving the multi-framework seam (model held constant, orchestration swapped by config). Optional extra `frameworks-openai`. 32 hermetic tests.
- **`Middleware/RiskAnalytics-API/graphs/project_risk_manager`** is a real **LangGraph map-reduce fan-out**: one worker per agent spec (`Send`), findings merged, per-agent errors captured (one failing specialist doesn't abort the run). **Verified live with a 7-detector fan-out** (22 findings across all risk dimensions in one run).
- **`Airflow/dags/` — 7 DAGs** driving the FastAPI boundaries over HTTP (no DB/LLM access; pure fake-client-testable task logic; 114 tests). Dataset ops (`schedule=None`) → Ingestion-API: `project_dataset_ingest`, `_acquire`, `_validate`, `_index`, `_reconcile`. Scheduled risk → RiskAnalytics-API: `project_risk_schedule` (daily, calls the existing per-project analysis endpoint), `portfolio_risk_schedule` (weekly, models the planned portfolio endpoint). Several acquire/validate/index/reconcile Ingestion-API endpoints and the portfolio endpoint are marked PENDING in the DAG code — they don't exist in middleware yet. Run Airflow: `WITH_AIRFLOW=1 bash CICD/LocalDev/docker-all-up.sh`.
- **`Portals/` — two Angular 20 apps** (`ng build` + unit tests pass). `Project-Tracker` (:4201, projects list over Projects-API) and `Admin` (:4200, agent config + framework toggle + audit + system health over Admin-API). Each: `npm start` = `ng serve`, multi-stage Dockerfile; started via `portals.sh`.
...
Still to build: the **8 finding-processor/reporter agents** (`risk_scoring`, `evidence_validation`, `risk_correlation`, `risk_deduplication`, `mitigation_planning`, `critic`, `project_reporting`, `executive_reporting`) — these consume `RiskFinding[]` (not evidence), so they need a **new port** + a multi-stage "risk review" LangGraph pipeline (detect → score → validate → correlate → dedup → mitigate → critic → report). Also: the middleware endpoints the DAGs mark PENDING; other alternative-framework adapters (strands/google_adk/ms_agent_framework — **CrewAI does not install on Python 3.14** due to a stale tiktoken pin); portfolio analysis; `docs/`.
