...
- **`Agents/` — the specialist-agent package (`epi-agents`).** `agent_core` = framework-agnostic port (`EvidencePackage → RiskFinding[]`); `schedule_risk/` = first implemented agent with a **LangGraph adapter** (real, default) + stub adapters for the other 5 frameworks + `registry.py` (the toggle). RiskAnalytics installs it via `local-deps.txt`. 6 hermetic tests. The other 15 agents are not built yet.
...
Still placeholders (dir + README only): `Airflow/`, `Portals/`, `docs/`, and 15 of 16 agents.
