...
- **`Middleware/Projects-API/` (:8003) — queries over the MongoDB evidence store** (pymongo). Search/get/metrics endpoints, **plus deterministic metric computation** (`POST /projects/{key}/metrics/compute` and `/metrics/compute-all`): reads ingested issues/histories/links/comments and computes 8 facts — backlog growth, reopen rate, blocker count, dependency depth, issue aging, resolution velocity, contributor concentration, critical-defect ratio (pure Python + Mongo, **no LLM**), upserting `project_metrics` + project counts. This turns raw ingested records into the evidence the agents consume (`agent_core.EvidenceMetrics`). 22 tests. Verified live (metrics match seeded evidence; agents cite the real numbers).
...
Functionally complete against the README vision. Intentionally deferred (add incrementally): the other alternative-framework adapters (`strands`/`google_adk`/`ms_agent_framework`; **CrewAI now installs on Python 3.12** but the team chose to keep everything on LangGraph for now); time-series metric facts (defect/velocity *trends* over multiple windows — the engine computes point-in-time facts today); and richer `docs/` content. **Metric computation assumes the ingested schema** (field names in `daos/metrics_computation`) — tune once the real archive layout is known.
