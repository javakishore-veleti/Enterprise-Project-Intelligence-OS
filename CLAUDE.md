...
Functionally complete against the README vision. Intentionally deferred (add incrementally): the other alternative-framework adapters (`strands`/`google_adk`/`ms_agent_framework`; **CrewAI now installs on Python 3.12** but the team chose to keep everything on LangGraph for now); and richer `docs/` content. **Metric computation assumes the ingested schema** — the ingest DAG normalizes the real `mongodump` layout (standard Jira issue docs with embedded changelog/comments/links), but the field mapping in `Airflow/.../project_dataset_ingest/tasks.py::transform_issue` + `daos/metrics_computation` should be confirmed against a full local restore (a 5.8 GB download → ~60 GB restore) when running the true dataset end-to-end. Already landed since the first cut: LangSmith tracing, opt-in API-key auth, velocity-trend + metrics-history, and the Project-Tracker UI depth.
