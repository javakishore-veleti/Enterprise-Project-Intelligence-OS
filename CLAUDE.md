...
- **`Middleware/Projects-API/` (:8003) — queries over the MongoDB evidence store** (pymongo). Search/get/metrics endpoints, **plus deterministic metric computation** (`POST /projects/{key}/metrics/compute` and `/metrics/compute-all`): reads ingested issues/histories/links/comments and computes 8 facts — backlog growth, reopen rate, blocker count, dependency depth, issue aging, resolution velocity, contributor concentration, critical-defect ratio (pure Python + Mongo, **no LLM**), upserting `project_metrics` + project counts. Also `resolution_velocity_trend` (recent vs prior window) and a **metrics-history time series** (`GET /projects/{key}/metrics/history` — each compute snapshots by `computed_at`). This turns raw ingested records into the evidence the agents consume (`agent_core.EvidenceMetrics`). 24 tests. Verified live (metrics match seeded evidence; agents cite the real numbers; multi-snapshot history).
...
- **`Middleware/RiskAnalytics-API/` (:8004) — multi-agent risk analysis, the core value.** Spans both stores: reads agent config from Postgres (`admin.agent_configs`), builds deterministic evidence from Mongo, runs specialists via the framework toggle, persists findings + reports to `risk.*`. `POST /api/v1/analysis/projects/{key}` (with optional `include_review`) → LangGraph → Claude → typed `RiskFinding[]` (+ reports); `POST /api/v1/analysis/portfolios/{key}` (cross-project); `GET /api/v1/analysis/runs/{id}`; `GET /api/v1/analysis/projects/{key}/runs` (run history with per-run finding/report counts, for the Project-Tracker history view). 28 tests (fakes, no LLM). **Verified live against real Claude** (7-detector fan-out, full review pipeline, 4-project portfolio). Requires `ANTHROPIC_API_KEY` in env at runtime.
...
- **`Airflow/dags/` — 8 DAGs** (pure fake-client-testable task logic; 88 tests). `project_dataset_acquire` = **real Zenodo download + md5**; `project_dataset_ingest` = **mongorestore + normalize** — the real archive is a `mongodump` (JiraReposAnon), so it restores into a `jira_repos` staging DB then normalizes Jira issues → evidence rows (issues/histories/comments/links) via **dynamic-mapped per-repo loaders**, then finalize marks the run COMPLETED (auto-triggers `metrics_compute`). Needs `mongodb-database-tools` + `pymongo` on the worker — **baked into the custom Airflow image** (`CICD/LocalDev/Airflow/Dockerfile`, tag `epi-airflow`), since the stock image lacks them and `PIP_ADDITIONAL_REQUIREMENTS` is ignored under `command: standalone`; `project_metrics_compute` = fan-out recompute of metrics via Projects-API; `_validate`/`_index`/`_reconcile` call Ingestion-API ops; `project_risk_schedule` (daily) / `portfolio_risk_schedule` (weekly) → RiskAnalytics-API. Only `acquire`+`ingest` write data; the rest are HTTP-only. Run Airflow: `WITH_AIRFLOW=1 bash CICD/LocalDev/docker-all-up.sh`.
- **`Portals/` — two Angular 20 apps** (`ng build` + unit tests pass). `Project-Tracker` (:4201): projects list + a **Project Risk view** that runs analyses (optionally with the review pipeline), renders findings + reports, and shows a **computed-metrics panel** (all 8 facts), an **analysis-run history** (`GET /analysis/projects/{key}/runs`) with drill-in, and a **per-agent breakdown** of a run. `Admin` (:4200): agent config + framework toggle + audit + system health, plus **Data Management → Initial Dataset** to trigger + monitor the dataset **download and batch ingestion** (live per-entity progress). Each: `npm start` = `ng serve`, multi-stage Dockerfile; started via `portals.sh`.
- **Observability:** RiskAnalytics wires **LangSmith tracing** (`LANGSMITH_TRACING` + `LANGSMITH_PROJECT`, key from env) — LangChain/LangGraph auto-trace every agent LLM call, the detector fan-out, and the review pipeline under one project. Verified live (traces landing in LangSmith).
- **Security:** opt-in **API-key auth** on all four services (`AUTH_ENABLED` + `API_KEY`) — business endpoints require an `X-API-Key` header when enabled; health stays public; off by default (anonymous principal) for local dev.
...
**Batch ingestion into Mongo** (Admin portal → Data Management → Initial Dataset → **"Ingest into evidence store"**, enabled once `DOWNLOADED`): Admin-API `POST /admin/dataset/ingest` → Ingestion-API creates a run + **triggers the Airflow `project_dataset_ingest` DAG**. The real dataset is a **MongoDB dump** (confirmed by reading the zip central directory over HTTP range — no full download), so the DAG: extract zip → **`mongorestore`** the archive into a `jira_repos` staging DB → **normalize** each repo's Jira issues into our evidence collections (issues/histories/comments/links/projects) via `dynamic-mapped` per-repo loaders — **bounded batches, idempotent upserts, resume via committed-batch skip**, checkpoints/progress through the governed Ingestion-API boundary (`ingestion_batches`/`ingestion_log`, V007). Never held in memory; evidence writes go straight to Mongo, status stays governed. `transform_issue` is a pure, tested function; **verified live against the real 337k-issue dataset** (bounded 10-repo ingest → normalize → evidence → metrics → 7-detector analysis, all grounded). Requires **`mongodb-database-tools`** + `pymongo` on the Airflow worker — baked into the custom `epi-airflow` image (`CICD/LocalDev/Airflow/Dockerfile`). The UI polls aggregated progress (`GET /admin/dataset/ingestion`). A completed run **auto-triggers metric computation**.

**End-to-end data path is complete and auto-chained:** download (Zenodo) → batch-ingest records into Mongo → **compute deterministic metrics** (a completed ingestion run **auto-triggers** `project_metrics_compute` via the Ingestion-API, `AUTO_COMPUTE_METRICS` default on, best-effort) → agents analyze real evidence. The seed evidence (`Database/MongoDB/seed/002_sample_evidence.js`) exercises this before the real 5.8 GB archive is loaded.

Functionally complete against the README vision. Intentionally deferred (add incrementally): the other alternative-framework adapters (`strands`/`google_adk`/`ms_agent_framework`; **CrewAI now installs on Python 3.12** but the team chose to keep everything on LangGraph for now); and richer `docs/` content. **The ingest schema mapping is confirmed against the real dataset.** The 5.8 GB archive was downloaded end-to-end through the governed path (Admin/Ingestion-API → Airflow `project_dataset_acquire` → local disk, md5-verified), then one repo (Mindville, ~2.1k issues) was `mongorestore`d and probed with `Airflow/.../project_dataset_ingest/probe_schema.py`. Result: the dump is `mongodump --db=JiraReposAnon --gzip --archive` (one collection per repo), and `transform_issue`'s paths map correctly — `key`, `fields.status.name`, `fields.created` at 100%; `fields.priority.name`/`resolutiondate`/`issuelinks` present where expected; `changelog.histories[].items[].toString` (status changes) and `issuelinks[].type.name`+`outwardIssue.key` verified on a real doc. **One real finding: the dataset carries NO `fields.comment`** — comments are absent dataset-wide, so comment evidence is always empty and `contributor_concentration` (which unions `comments`+`issue_histories` authors in `daos/metrics_computation`) falls back to history authors only. The confirmed shape is regression-locked by `Airflow/tests/fixtures/real_jira_issue.json` (synthetic values, faithful structure — the dataset itself stays out of the repo per its license). A full ~60 GB restore of all 16 repos is still only needed to load the complete corpus, not to trust the mapping. Already landed since the first cut: LangSmith tracing, opt-in API-key auth, velocity-trend + metrics-history, the Project-Tracker UI depth, and this real-dataset schema confirmation.
