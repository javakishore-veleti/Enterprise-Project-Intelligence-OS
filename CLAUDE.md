...
Functionally complete against the README vision. Intentionally deferred (add incrementally): the other alternative-framework adapters (`strands`/`google_adk`/`ms_agent_framework` for detectors; framework variants for review agents — **CrewAI now installs on Python 3.12** and can be added); extracting/ingesting the 2.7M downloaded records into Mongo (the download marks the dataset DOWNLOADED; ingestion of records is the next step); and richer `docs/` content.

### Running an analysis locally (needs the LLM)
`ANTHROPIC_API_KEY` must be in the environment (the model SDK reads it; it is never stored/committed). The `api-services.sh` launcher inherits the shell env. Flow: containers up → `npm run local:db:migrate` (V001–V006) → seed Mongo → `local:api-services:start-all` → `POST /api/v1/analysis/projects/APACHE` (add `{"include_review": true}` for the full pipeline + reports; or `POST /api/v1/analysis/portfolios/{key}` for cross-project). The per-agent model/framework comes from `admin.agent_configs` (set via Admin-API `PUT /api/v1/admin/agents/{key}`).
