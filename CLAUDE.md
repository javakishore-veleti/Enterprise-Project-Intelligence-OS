...
- **`Airflow/dags/` — 7 DAGs** (pure fake-client-testable task logic; 85 tests). `project_dataset_acquire` does the **real Zenodo download + md5**; `project_dataset_ingest` does **real, batched, resumable ingestion into Mongo** (extract → discover → dynamic-mapped per-entity loaders → checkpoints → reconcile). `_validate`/`_index`/`_reconcile` call the Ingestion-API ops. Scheduled risk → RiskAnalytics-API: `project_risk_schedule` (daily), `portfolio_risk_schedule` (weekly). Only `acquire`+`ingest` write data (download/Mongo); the rest are HTTP-only. Run Airflow: `WITH_AIRFLOW=1 bash CICD/LocalDev/docker-all-up.sh`.
...
Functionally complete against the README vision. Intentionally deferred (add incrementally): the other alternative-framework adapters (`strands`/`google_adk`/`ms_agent_framework` for detectors; framework variants for review agents — **CrewAI now installs on Python 3.12** and can be added); computing/refreshing project metrics from the ingested records; and richer `docs/` content.
