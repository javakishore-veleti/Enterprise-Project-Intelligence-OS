...
- **`Airflow/dags/` — 7 DAGs** driving the FastAPI boundaries over HTTP (no DB/LLM access; pure fake-client-testable task logic; 114 tests). Dataset ops (`schedule=None`) → Ingestion-API: `project_dataset_ingest`, `_acquire`, `_validate`, `_index`, `_reconcile` — **all their endpoints now exist**. Scheduled risk → RiskAnalytics-API: `project_risk_schedule` (daily), `portfolio_risk_schedule` (weekly) — **both endpoints now exist too**. Run Airflow: `WITH_AIRFLOW=1 bash CICD/LocalDev/docker-all-up.sh`.
- **`Portals/` — two Angular 20 apps** (`ng build` + unit tests pass). `Project-Tracker` (:4201): projects list over Projects-API **plus a Project Risk view** that runs an analysis (optionally with the review pipeline) over RiskAnalytics-API and renders findings + the generated reports. `Admin` (:4200): agent config + framework toggle + audit + system health over Admin-API. Each: `npm start` = `ng serve`, multi-stage Dockerfile; started via `portals.sh`.
- `Database/PostgreSQL/` migrations `V001`–`V005` (ingestion runs; admin config+audit seeding 16 agents; risk runs+findings; findings `meta`+`risk.reports`; `ingestion.operations`) + `apply_migrations.py` (idempotent, pg8000); `Database/MongoDB/` init + indexes + local-dev `seed/`.
- `OpenAPI/{ingestion,projects,admin,risk-analytics}-api.yaml` generated from the services.

Functionally complete against the README vision. Intentionally deferred (add incrementally): the other alternative-framework adapters (`strands`/`google_adk`/`ms_agent_framework` for detectors; framework variants for review agents — **CrewAI does not install on Python 3.14** due to a stale tiktoken pin); real dataset acquisition/download (acquire/verify/extract are stubbed — no live 5.8 GB dataset in this environment); and richer `docs/` content.

### Running an analysis locally (needs the LLM)
`ANTHROPIC_API_KEY` must be in the environment (the model SDK reads it; it is never stored/committed). The `api-services.sh` launcher inherits the shell env. Flow: containers up → `npm run local:db:migrate` (V001–V005) → seed Mongo → `local:api-services:start-all` → `POST /api/v1/analysis/projects/APACHE` (add `{"include_review": true}` for the full pipeline + reports; or `POST /api/v1/analysis/portfolios/{key}` for cross-project). The per-agent model/framework comes from `admin.agent_configs` (set via Admin-API `PUT /api/v1/admin/agents/{key}`).
