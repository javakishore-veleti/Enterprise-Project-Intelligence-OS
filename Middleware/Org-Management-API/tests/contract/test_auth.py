...
from org_management_api.common.security import verify_api_key
...
def test_health_public_and_business_secured(monkeypatch) -> None:
    ...
    try:
        ...
        # raise_server_exceptions=False: the valid-key call may fail downstream
        # on the DB (no infra in tests) -> we only care it clears the auth gate.
        client = TestClient(create_app(), raise_server_exceptions=False)
        assert client.get("/health/live").status_code == 200          # public
        r = client.get("/api/v1/orgs")                                 # no key
        assert r.status_code == 401
        # a valid key passes the auth gate (may then fail on DB, but not 401)
        r2 = client.get("/api/v1/orgs", headers={"X-API-Key": "secret"})
