"""Unit tests for the Phase-2 org-scope seam: the HTTP org-access gateway
(parsing + graceful degradation), the ``narrow_with_org_scope`` composition, and
the ``provide_org_scope`` header dependency. All hermetic — ``urlopen`` is
...
from projects_api.api.dependencies import provide_org_scope
...
def test_narrow_org_scope_only_becomes_the_scope() -> None:
    scope = OrgScope(project_keys=("A", "B"))
    assert narrow_with_org_scope(None, scope) == ["A", "B"]


def test_narrow_ands_org_scope_with_assignment_keys() -> None:
    # intersection, preserving the assignment order (deterministic)
