...
def _to_response(doc: dict) -> ProjectMetricsResponse:
    return ProjectMetricsResponse(
        ...
        resolution_velocity_trend=doc.get("resolution_velocity_trend", 0.0),
...
class MongoProjectMetricsDao(ProjectMetricsDao):
    ...
    def latest(self, project_key: str) -> ProjectMetricsResponse | None:
        doc = self._db.db()["project_metrics"].find_one(
            {"project_key": project_key}, sort=[("computed_at", DESCENDING)])
        return _to_response(doc) if doc else None

    def history(self, project_key: str, limit: int) -> list[ProjectMetricsResponse]:
        cursor = (self._db.db()["project_metrics"]
                  .find({"project_key": project_key})
                  .sort("computed_at", DESCENDING).limit(limit))
        return [_to_response(d) for d in cursor]
