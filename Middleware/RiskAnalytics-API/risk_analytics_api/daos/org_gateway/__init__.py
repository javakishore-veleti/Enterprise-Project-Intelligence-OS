...
Phase-2 multi-tenancy seam (ported from Projects-API). Read-only: resolves the
set of project keys a user or org context may see, so the risk-analysis read/run
path can scope results to it.

Uses stdlib ``urllib`` (same convention as the other cross-service gateways — no
...
def _project_keys(body: dict) -> list[str]:
    """Extract de-duplicated ``external_key`` values (== our ``project_key``) from
    an effective/visible-projects resolver response, preserving order."""
    seen: dict[str, None] = {}
