...
from admin_api.common.security import verify_api_key
...
def test_health_public_business_secured(monkeypatch) -> None:
    ...
    try:
        ...
        assert c.get("/api/v1/admin/agents").status_code == 401
        assert c.get("/api/v1/admin/agents", headers={"X-API-Key": "secret"}).status_code != 401
    finally:
        get_settings.cache_clear()
